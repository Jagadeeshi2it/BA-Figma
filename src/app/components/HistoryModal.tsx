...
const HistoryModal = memo(React.forwardRef<HTMLDivElement, HistoryModalProps>(({ open, onOpenChange, history, doorShelfConfig: currentDoorConfig }, ref) => {
  ...
  const renderSourceBinInfo = (entry: AllocationHistoryEntry, product: any) => {
    if (entry.sourceBin) {
      // CRITICAL FIX: Use product.quantity directly - it already represents the total moved for THIS product entry
      // DO NOT sum from bins array as that causes multiplication when products are deduplicated
      const totalMovedFromSource = product.quantity || 0;
      ...
      return (
        <>
          ...
          <div className="text-sm text-gray-600 mt-1 flex gap-4">
            <span className="text-[14px]">Moved: <span className="text-green-700">{totalMovedFromSource} {pluralizeUnit(product.unit, totalMovedFromSource)}</span></span>
          ...
          {filteredHistory.length === 0 ? (
            ...
            <div className="space-y-[8px]">
              {filteredHistory.slice(0, 50).map((entry) => {
                ...
                // CRITICAL FIX: Enhanced product deduplication to prevent double entries
                // This ensures that when multiple bins are involved in a transaction,
                // products are properly consolidated into single entries
                    ...
                    <div key={entry.id} className="bg-white rounded-lg border p-6">
                      ...
                      <div className="space-y-6">
                        {uniqueProducts.map((product, productIndex) => {
                          ...
                          const rawAssignedBins = (product as any).targetBins || entry.bins;
                          
                          // CRITICAL FIX: Enhanced bin deduplication to prevent double entries
                          // This ensures that when multiple bins are involved in a transaction,
                          // duplicate bin entries are properly consolidated
                          const binMap = new Map<string, any>();
                          const assignedBins = rawAssignedBins.reduce((acc: any[], bin: any) => {
                            // Create location-based key for identifying the same physical bin
                            const normalizedBinId = String(bin.binId || 'unknown');
                            const normalizedCabinetNumber = String(bin.cabinetNumber || 'unknown');
                            const normalizedDoorNumber = String(bin.doorNumber || 'unknown');
                            const normalizedShelfName = String(bin.shelfName || 'unknown');
                            const normalizedBinName = String(bin.binName || 'unknown');
                            
                            const locationKey = `${normalizedBinId}|${normalizedCabinetNumber}|${normalizedDoorNumber}|${normalizedShelfName}|${normalizedBinName}`;
                            
                            const existingBin = binMap.get(locationKey);
                            
                            if (existingBin) {
                              // Merge quantities if it's the same physical bin with different quantities
                              const existingQty = Number(existingBin.quantity) || 0;
                              const newQty = Number(bin.quantity) || 0;
                              const existingExistingQty = Number(existingBin.existingQuantity) || 0;
                              const newExistingQty = Number(bin.existingQuantity) || 0;
                              
                              // Only merge if quantities are different, otherwise it's a true duplicate
                              if (existingQty !== newQty || existingExistingQty !== newExistingQty) {
                                existingBin.quantity = existingQty + newQty;
                                existingBin.existingQuantity = Math.max(existingExistingQty, newExistingQty);
                              }
                              // Don't add duplicate - just update existing
                            } else {
                              // New bin location
                              binMap.set(locationKey, { ...bin });
                              acc.push({ ...bin });
                            }
                                      ...
                                      <div className="space-y-3">
                                        ...
                                        <div>
                                          ...
                                          {assignedBins.length > 0 ? assignedBins.map((bin, binIndex) => (
                                              <div key={`${entry.id}-product-${product.id}-bin-${String(bin.binId || 'unknown')}-${String(bin.cabinetNumber || 'unknown')}-${String(bin.doorNumber || 'unknown')}-${binIndex}`} className="mb-2">
                                                <div className="font-medium text-gray-900">
                                                  {bin.binName}
                                                ...
                                                {isProductMoved && (
                                                  <div className="text-xs text-[rgba(74,85,101,1)] mt-1 text-[14px] flex gap-4">
                                                    {(() => {
                                                      ...
                                                      // Debug logging for existing quantity display
                                                      console.log('🔍 HistoryModal Existing Quantity Display:', {
                                                        entryId: entry.id,
                                                        ...
                                                        binData: {
                                                          ...
                                                          existingQuantity: bin.existingQuantity,
                                                          rawExistingQuantity: bin.existingQuantity
                                                        },
                                                        calculatedValues: {
                                                          movedQty,
                                                          existingQty,
                                    ...
                                    {isProductMoved && (
                                      <div className="text-xs text-[rgba(74,85,101,1)] mt-1 text-[14px] flex gap-4">
                                        {(() => {
                                          ...
                                          // Debug logging for existing quantity display (single product)
                                          console.log('🔍 HistoryModal Single Product Existing Quantity Display:', {
                                            entryId: entry.id,
                                            productId: product.id,
                                            binId: bin.binId,
                                            binName: bin.binName,
